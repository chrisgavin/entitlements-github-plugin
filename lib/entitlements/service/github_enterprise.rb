...
module Entitlements
  class Service
    class GitHubEnterpriseService
      ...
      attr_reader :addr, :enterprise, :token, :ou

      # Constructor.
      #
      # addr          - Base URL a GitHub Enterprise Server API (leave undefined to use dotcom)
      # enterprise    - String with enterprise name
      ...
      def initialize(addr: nil, enterprise:, token:, ou:)
        ...
        # need to be obtained only one time per enterprise, but might be used multiple times.
      ...
      # Read the members of an enterprise and return a Hash of users with their role.
      # This method does not need parameters because the underlying service already
      # has the enterprise available in an `enterprise` method.
      #
      # Takes no parameters.
      #
      ...
      def enterprise_members
        Entitlements.cache[:github_enterprise_members][enterprise_signature] ||= begin
          roles = Entitlements::Backend::GitHubEnterprise::ENTERPRISE_ROLES.invert
          ...
          counts = results.map { |_, role| role }.tally
          Entitlements.logger.debug "Currently #{enterprise} has #{counts}"

          { cache: cache, value: result }
        end

        Entitlements.cache[:github_enterprise_members][enterprise_signature][:value]
      end
      ...
      # Read the members of an enterprise who are in a "pending" role. These users should
      ...
      def pending_members
        Entitlements.cache[:github_pending_members][enterprise_signature] ||= begin
          # On Enterprise server, users are just added, not invited.
          return Set.new if enterprise?
          pm = pending_members_from_graphql
          Entitlements.logger.debug "Currently #{enterprise} has #{pm.size} pending member(s)"
          pm
        end
      end
      ...
      def org_members_from_predictive_cache?
        enterprise_members # Force this to be read if for some reason it has not been yet.
        Entitlements.cache[:github_enterprise_members][enterprise_signature][:cache] || false
      end

      # Invalidate the predictive cache for enterprise members, and if the prior knowledge
      ...
      # Returns nothing.
      Contract C::None => nil
      def invalidate_enterprise_members_predictive_cache
        ...
        return unless enterprise_members_from_predictive_cache?

        # The entry did come from the predictive cache. Invalidate the entry, clear local
        # caches, and re-read the data from the API.
        ROLES.each do |role|
          Entitlements.logger.debug "Invalidating cache entries for cn=#{role},#{ou}"
          Entitlements::Data::Groups::Cached.invalidate("cn=#{role},#{ou}")
        end
        Entitlements.cache[:github_enterprise_members].delete(enterprise_signature)
        enterprise_members
        nil
      end
      ...
      def members_and_roles_from_graphql
        Entitlements.logger.debug "Loading enterprise members and roles for #{enterprise}"

  ...
  class Backend
    class GitHubEnterprise
      class Controller < Entitlements::Backend::BaseController
        ...
        ROLES = Entitlements::Backend::GitHubEnterprise::ENTERPRISE_ROLES.keys.freeze
        ...
        def initialize(group_name, config = nil)
          super
          @provider = Entitlements::Backend::GitHubEnterprise::Provider.new(config: @config)
        end
        ...
        def apply(action)
          unless action.existing.is_a?(Entitlements::Models::Group) && action.updated.is_a?(Entitlements::Models::Group)
            logger.fatal "#{action.dn}: GitHub entitlements interface does not support creating or removing a GitHub enterprise"
            raise RuntimeError, "Invalid Operation"
          end

          if provider.commit(action)
            logger.debug "APPLY: Updating GitHub enterprise #{action.dn}"
          else
        ...
        def validate_config!(key, data)
          ...
          text = "GitHub enterprise group #{key.inspect}"
        ...
        Contract C::None => C::Any
        def validate_github_enterprise_ous!
          ...
          ROLES.each do |role|
            ...
            unless updated.member?(role_dn)
              logger.fatal "GitHubEnterprise: No group definition for #{group_name}:#{role} - abort!"
              raise "GitHubEnterprise must define #{ROLES.join(', ')} roles."
            end
          end
          ...
          logger.fatal "GitHubEnterprise: Unexpected role(s) in #{group_name}: #{unexpected_roles.join(', ')}"
          raise "GitHubEnterprise unexpected roles."
        end

        # Validate that within a given GitHub enterprise, a given person is not assigned to multiple
        ...
        def validate_no_dupes!
          ...
          ROLES.each do |role|
            ...
            if dupes.empty?
              ...
              raise Entitlements::Backend::GitHubEnterprise::DuplicateUserError, "Abort due to users in multiple roles"
            end
          end
        end
        ...
        def changes
          ...
          begin
            ...
            # are being ignored or enterprise membership is pending.
            groups = ROLES.map do |role|
              role_dn = ["cn=#{role}", config.fetch("base")].join(",")
              [role, Entitlements::Data::Groups::Calculated.read(role_dn)]
            end.to_h

            # Categorize changes by :added (invite user to the enterprise), :moved (change a user's role), and
            # :removed (remove a user from the enterprise). This operates across all roles.
